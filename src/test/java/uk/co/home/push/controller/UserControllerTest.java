...
public class UserControllerTest {
    ...
    public void itShouldReturnGivenUsers() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/users").contentType(MediaType.APPLICATION_JSON)).andDo(print()).andExpect(status().isOk())
    ...
    public void itShouldCreateNewUser() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/users/create").contentType(MediaType.APPLICATION_JSON).content(mapper.writeValueAsString(createRequest)))
                .andDo(print()).andExpect(status().isCreated()).andExpect(header().string(CACHE_CONTROL, CACHE_HEADER_VALUES))
    ...
    public void itShouldFail_BadRequest() throws Exception {
        var createRequest = CreateUserRequest.Builder.create().withUsername("").withAccessToken(AN_ACCESS_TOKEN).build();
    ...
    public void itShouldFail_Conflict() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/users/create").contentType(MediaType.APPLICATION_JSON).content(mapper.writeValueAsString(createRequest)))
                .andDo(print()).andExpect(status().isConflict()).andExpect(header().string(CACHE_CONTROL, CACHE_HEADER_VALUES))
                .andExpect(header().string(PRAGMA, NO_CACHE)).andExpect(header().string(EXPIRES, "0"))
                .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
    ...
    public void itWill405OnAnyNonGET_DELETE() throws Exception {
        mockMvc.perform(delete("/api/v1/users")).andExpect(status().isMethodNotAllowed());
    }

    @Test
    public void itWill405OnAnyNonGET_PUT() throws Exception {
        mockMvc.perform(put("/api/v1/users")).andExpect(status().isMethodNotAllowed());
    }
