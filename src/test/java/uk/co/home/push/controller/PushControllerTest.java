...
public class PushControllerTest {
    ...
    public void itShouldPushNotification() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/push").contentType(MediaType.APPLICATION_JSON).content(mapper.writeValueAsString(pushNotificationRequest)))
                .andDo(print()).andExpect(status().isOk()).andExpect(header().string(CACHE_CONTROL, CACHE_HEADER_VALUES))
                .andExpect(header().string(PRAGMA, NO_CACHE)).andExpect(header().string(EXPIRES, "0"))
    ...
    public void itShouldBadRequest_MissingValue() throws Exception {
        var pushNotificationRequest = PushNotificationRequest.Builder.create().withUsername("").withTitle(A_TITLE).withBody(A_MESSAGE).build();

        mockMvc.perform(post("/api/v1/push").contentType(MediaType.APPLICATION_JSON).content(mapper.writeValueAsString(pushNotificationRequest)))
                .andDo(print()).andExpect(status().isBadRequest()).andExpect(header().string(CACHE_CONTROL, CACHE_HEADER_VALUES))
                .andExpect(header().string(PRAGMA, NO_CACHE)).andExpect(header().string(EXPIRES, "0"))
                .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
    ...
    public void itShouldFail_PushApi() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/push").contentType(MediaType.APPLICATION_JSON).content(mapper.writeValueAsString(pushNotificationRequest)))
                .andDo(print()).andExpect(status().isInternalServerError()).andExpect(header().string(CACHE_CONTROL, CACHE_HEADER_VALUES))
                .andExpect(header().string(PRAGMA, NO_CACHE)).andExpect(header().string(EXPIRES, "0"))
                .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
    ...
    public void itShouldFail_Increment() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/push").contentType(MediaType.APPLICATION_JSON).content(mapper.writeValueAsString(pushNotificationRequest)))
                .andDo(print()).andExpect(status().isInternalServerError()).andExpect(header().string(CACHE_CONTROL, CACHE_HEADER_VALUES))
                .andExpect(header().string(PRAGMA, NO_CACHE)).andExpect(header().string(EXPIRES, "0"))
                .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON)).andExpect(content().string(
                        mapper.writeValueAsString(new ApiStatus(HttpStatus.INTERNAL_SERVER_ERROR, "Incrementing notification count failure"))));
    ...
    public void itWill405OnAnyNonGET_DELETE() throws Exception {
        mockMvc.perform(delete("/api/v1/push")).andExpect(status().isMethodNotAllowed());
    }

    @Test
    public void itWill405OnAnyNonGET_PUT() throws Exception {
        mockMvc.perform(put("/api/v1/push")).andExpect(status().isMethodNotAllowed());
    }
