...
public class PushNotificationHandler {
	...
	public void pushNotification(User user, PushNotificationRequest pushNotificationRequest) throws ExecutionException, InterruptedException {
		...
		logger.info("Sending a push notification to pushbullet API for user {} with title {} ", user.getUsername(), simplePushNotification.getTitle());
		Mono<ResponseEntity<String>> pushNotificationResponse = webClient.post()
					.uri("/v2/pushes")
					.body(Mono.just(simplePushNotification), SimplePushNotification.class)
					.header("Access-Token", user.getAccessToken())
		...
		if (response.getStatusCode().is4xxClientError()) {
			throw new BadRequestException("Push API -> Status : " + response.getStatusCode() + ", Error : " + response.getStatusCode().getReasonPhrase());
		} else if (response.getStatusCode().is5xxServerError()) {
			throw new RuntimeException("Push API -> Status : " + response.getStatusCode() + ", Error : " + response.getStatusCode().getReasonPhrase());
		}
	}

	private SimplePushNotification newSimplePushNotification(PushNotificationRequest pushNotificationRequest) {
		return SimplePushNotification.Builder.create()
											.withTitle(pushNotificationRequest.getTitle())
											.withBody(pushNotificationRequest.getBody()).build();
	}
